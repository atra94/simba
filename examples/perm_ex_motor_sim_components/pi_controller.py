...
class PIController(SystemComponent):
    ...
    def __init__(self, name='pi_controller', p_gain=1.0, i_gain=0.01):
        self._p_gain = p_gain
        self._i_gain = i_gain
        error_input = Input(self, name='error', size=1, dtype=float_base_type)
        output = Output(
            self, name='action', dtype=float_base_type, size=1, signal_names=('action',), component_inputs=(error_input,)
        )
