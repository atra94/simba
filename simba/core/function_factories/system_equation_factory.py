...
def create_system_equation(state_functions, output_functions, state_length, global_extra_type):

    fct = _create_arbitrary_function(state_functions, output_functions, state_length)
    if all(type(fct_) == nb.core.registry.CPUDispatcher for fct_ in list(state_functions) + list(output_functions)):
        signature = float_array(float_base_type, float_array, float_array, int_array, global_extra_type)
        fct = nb.njit(signature)(fct)
    return fct


def _create_arbitrary_function(state_functions, output_functions, state_length):
    """
    ...
    state_reader = ""
    output_reader = ""
    header = "def system_equation(t, global_state, global_float_outputs, global_int_outputs, global_extras):\n"
